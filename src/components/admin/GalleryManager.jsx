    ...
    try {
      if (!formData.image_url) {
        toast({
          variant: "destructive",
          title: "Falta imagen",
          description: "Debes subir una imagen para la galería.",
        });
        return;
      }
  ...
  const handleDelete = async (id) => {
    try {
      ...
      toast({
        ...
        description: "La imagen ha sido borrada correctamente.",
      });
      fetchImages();
    } catch (error) {
      toast({
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-stone-600">
            Sube y organiza las fotos del sitio
          </p>
        </div>
        <Button
      ...
      <Tabs defaultValue="all" className="space-y-6">
        <TabsList className="bg-white border border-stone-200 w-full justify-start p-2 h-auto flex-wrap gap-2">
          <TabsTrigger
            value="all"
            className="data-[state=active]:bg-stone-100 px-4 py-2"
          >
            Todas
          </TabsTrigger>
          {categories.map((cat) => (
  ...
  return (
    ...
    >
      <div className="aspect-square bg-stone-100 relative overflow-hidden">
        <motion.img
          layoutId={`image-${img.id}`} // <--- ESTO ES LA CLAVE
          src={img.image_url}
          alt={img.title}
          className="..."
        />
      ...
      <div className="p-3">
        <h4
          className="font-bold text-sm text-amber-900 truncate"
          title={img.title}
        >
          {img.title}
        </h4>
